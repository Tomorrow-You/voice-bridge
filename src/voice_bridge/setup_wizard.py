...
def run_setup():
    ...
    print("  Add this Stop hook to your .claude/settings.json:")
    print()
    print('  "hooks": {')
    print('    "Stop": [{')
    print('      "hooks": [{')
    print('        "type": "command",')
    print('        "command": "bash -c \'VB_HOOK=$(python3 -c \"import voice_bridge; import pathlib; print(pathlib.Path(voice_bridge.__file__).parent / \\\"integrations\\\" / \\\"claude_hook.sh\\\")\" 2>/dev/null) && [ -f \"$VB_HOOK\" ] && bash \"$VB_HOOK\"\'",')
    print('        "timeout": 5')
    print('      }]')
    print('    }]')
    print('  }')
